...
void ShopUI::Run() {
    if(m_TFBox->GetVisibility()){
        if(m_TFBox->Choose()){
            ...
            if (m_TFBox->GetTF()){
                if(m_Player->GetMoney()>=m_BuyList[m_RowTopIndex+(180-m_Arrows[1]->GetPosition().y)/90].second) {
                    m_TextBox->SetVisible(false);
        ...
        if(m_TextBox->GetVisibility()){
            ...
            m_TextBox->SetText("這樣總共是"+std::to_string(m_BuyList[m_RowTopIndex+(180-m_Arrows[1]->GetPosition().y)/90].second)+"元，您要購買嗎?");
